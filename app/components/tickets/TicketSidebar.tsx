...
interface TicketSidebarProps {
  ...
  // ✅ NEW PROP: Callback to update logs instantly
...
export default function TicketSidebar({ 
  ...
  onStatusChanged, onActivityLogAdded 
}: TicketSidebarProps) {
  ...
  // ✅ 3. Helper to Create Log Instantly (Optimistic Update)
  const createLocalLog = (action: string, description: string) => {
    const newLog: ActivityLog = {
      id: `temp-${Date.now()}`, // Temporary ID
      action: action,
      description: description,
      user: "You", // Placeholder until refresh
      timestamp: new Date().toISOString(),
      metadata: {}
    };
    
    // Send to Parent to update UI instantly
    if (onActivityLogAdded) {
      onActivityLogAdded(newLog);
    }
  };
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
            ...
            <div className="relative">
              ...
              {isPriorityOpen && (
                <div className="absolute top-full left-0 mt-1 w-32 bg-[#181A1F] border border-white/10 rounded-lg shadow-xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-100">
                  <div className="p-1">
                    <button onClick={() => handlePriorityChange("NORMAL")} className="w-full text-left px-3 py-2 text-xs text-gray-300 hover:bg-white/5 rounded flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-gray-400" /> Normal
                    </button>
                    <button onClick={() => handlePriorityChange("URGENT")} className="w-full text-left px-3 py-2 text-xs text-red-400 hover:bg-red-500/10 rounded flex items-center gap-2">
          ...
          <div className="flex items-center gap-4">
            <button onClick={onClose} className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-white transition-colors">
              <ChevronsRight size={14} /> <span>Collapse</span>
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4">
                ...
                <div className="absolute top-full left-0 mt-1 w-full bg-[#181A1F] border border-white/10 rounded-lg shadow-xl overflow-hidden z-20 animate-in fade-in zoom-in-95 duration-100">
                  <div className="p-1 space-y-0.5">
                    {[
                      ...
                      <button key={opt.val} onClick={() => handleStatusChange(opt.val)} className="w-full text-left px-3 py-2 text-xs text-gray-300 hover:bg-white/5 rounded flex items-center gap-2">
                         <div className={`w-1.5 h-1.5 rounded-full ${opt.col}`} /> {opt.label}
                      </button>
                    ))}
                  </div>
                </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center justify-between bg-[#181A1F] border border-white/10 rounded-lg px-3 py-2.5 cursor-pointer opacity-70">
                 <span className="text-gray-500 italic text-xs">Unassigned</span><ChevronDown size={14} className="text-gray-500" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#181A1F] border border-white/10 rounded-xl p-4 space-y-3">
             <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <span className="bg-blue-500/20 text-blue-400 text-xs font-bold px-2 py-0.5 rounded-full">{requirementsCount} items</span>
                </div>
                <div className="flex gap-2">
                    {ticket.extraction_status === "VALID" ? (
                      <button onClick={handleDownload} className="flex items-center gap-1.5 text-xs font-medium text-emerald-500 hover:text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 px-2 py-1 rounded transition-colors">
                        <Download size={12} /> Excel
                      </button>
                    ) : <span className="text-[10px] font-bold text-gray-600 uppercase tracking-wider">Irrelevant</span>}
                   <button onClick={onEditRequirements} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-500 text-white text-xs font-medium hover:bg-emerald-600 transition-colors shadow-lg shadow-emerald-500/20">
          ...
          <div className="space-y-2 border-t border-white/10 pt-4">
             ...
             <div className="border-b border-white/5 pb-2">
                <button onClick={() => toggleSection('files')} className="w-full flex items-center justify-between py-3 hover:text-white transition-colors group">
                    <div className="flex items-center gap-3 font-medium text-gray-300 group-hover:text-emerald-400"><FileCheck size={18} /><span>Quotation Files</span></div>
             ...
             {/* CPO */}
            <div className="border-b border-white/5 pb-2">
              <button onClick={() => toggleSection('cpo')} className="w-full flex items-center justify-between py-3 hover:text-white transition-colors group">
                  <div className="flex items-center gap-3 font-medium text-gray-300 group-hover:text-emerald-400"><ShoppingCart size={18} /><span>Customer Purchase Order (CPO)</span></div>
            ...
            {/* Internal Notes */}
             <div>
                <button onClick={() => toggleSection('notes')} className="w-full flex items-center justify-between py-3 hover:text-white transition-colors group">
                    <div className="flex items-center gap-3 font-medium text-gray-300 group-hover:text-emerald-400"><MessageSquare size={18} /><span>Internal Notes ({internalNotes.length})</span></div>
                    ...
                    <div className="mt-2 space-y-4 animate-in slide-in-from-top-2 duration-200">
                        <div className="space-y-2">
                          <textarea 
                              ...
                              className="w-full h-20 bg-[#0A0B0D] border border-white/10 rounded-lg p-3 text-sm text-gray-300 placeholder-gray-600 focus:outline-none focus:border-emerald-500/50 resize-none"
                              placeholder="Add an internal note..."
                          ></textarea>
                          <button onClick={handleAddNote} disabled={isSendingNote || !noteText.trim()} className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium py-2 rounded-lg transition-all shadow-lg shadow-emerald-900/20">
                              {isSendingNote ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />} Add Note
                          </button>
                        </div>
                        <div className="space-y-3 max-h-60 overflow-y-auto pr-1 custom-scrollbar">
                          {internalNotes.length > 0 ? (
                            internalNotes.map((note) => (
                              <div key={note.id} className="bg-[#0A0B0D] border border-white/5 rounded-lg p-3 space-y-1">
                                <div className="flex items-center justify-between text-xs text-gray-500">
                                  <span className="font-semibold text-emerald-500">{note.author}</span>
                                  <span>{new Date(note.created_at).toLocaleString()}</span>
                                </div>
                                <p className="text-sm text-gray-300 whitespace-pre-wrap">{note.text}</p>
                              </div>
                            ))
                          ) : <div className="text-center text-xs text-gray-600 italic py-2">No notes yet.</div>}
                        </div>
                    </div>
                )}
             </div>
             ...
             <div className="border-t border-white/5 pt-2">
                <button onClick={() => toggleSection('activity')} className="w-full flex items-center justify-between py-3 hover:text-white transition-colors group">
                    <div className="flex items-center gap-3 font-medium text-gray-300 group-hover:text-emerald-400"><History size={18} /><span>Activity Logs ({sortedLogs.length})</span></div>
                    {sections.activity ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                </button>
                {sections.activity && (
                    ...
                    {sortedLogs.length > 0 ? (
                      sortedLogs.map((log) => (
                        <div key={log.id} className="relative pl-6 pb-6 border-l border-white/10 last:border-0 last:pb-0 group">
                          <div className="absolute -left-[9px] top-0 w-5 h-5 rounded-full bg-[#181A1F] border border-white/10 flex items-center justify-center">
                            {getLogIcon(log.action)}
                          </div>
                          <div className="space-y-1">
                            <div className="flex items-center justify-between">
                              <span className="text-xs font-semibold text-gray-200">{log.user}</span>
                              <span className="text-[10px] text-gray-500">{format(new Date(log.timestamp), 'MMM d, h:mm a')}</span>
                            </div>
