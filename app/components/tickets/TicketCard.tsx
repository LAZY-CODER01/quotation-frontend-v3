  ...
  return (
    <div
      className={`
       ...
       bg-[rgb(var(--bg-secondary))] hover:shadow-lg 
        
        ${isUrgent
          ? 'border border-l-red-500 border-l-4 hover:border-red-500 hover:shadow-red-500/10 hover:bg-red-500/5'
          : 'border border-[rgb(var(--border-primary))] hover:border-[rgb(var(--color-emerald))] hover:shadow-[rgb(var(--color-emerald)_/_0.1)] hover:bg-[rgb(var(--color-emerald)_/_0.05)]'
        }
      `}
    >
      ...
      <div className="flex justify-between items-start">
        <h4 className="text-base font-semibold text-[rgb(var(--text-primary))] group-hover:text-[rgb(var(--color-emerald))] transition-colors truncate pr-2" title={companyName}>
          {companyName}
        </h4>
        <span className="text-xs font-medium text-[rgb(var(--text-secondary))] whitespace-nowrap shrink-0">
          {timeDisplay}
        </span>
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-[rgb(var(--text-secondary))] truncate mt-0.5" title={senderEmail}>
          {senderEmail}
        </p>
        ...
        {(() => {
          ...
          if (amountDisplay) {
            return (
              <div className={`mt-2 text-sm font-bold truncate ${isCPO ? 'text-[rgb(var(--color-blue))]' : 'text-[rgb(var(--color-emerald))]'}`}>
      ...
      <div className="mt-2 pt-2 border-t border-[rgb(var(--border-secondary))] flex items-center justify-between">
        <div className="flex items-center gap-3">
          {/* Items Count */}
          {data.extraction_status === "VALID" && (
            <div className="flex text-[10px] items-center gap-1 text-[rgb(var(--text-secondary))] group-hover:text-[rgb(var(--text-primary))] transition-colors">
        ...
        <div className={`flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded ${data.assigned_to ? "text-[rgb(var(--color-blue))] bg-[rgb(var(--color-blue)_/_0.1)]" : "text-[rgb(var(--text-tertiary))]"}`}>
          <User size={10} />
